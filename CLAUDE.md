...
**Everything under `src/assets/images/` ships to production, referenced or not.** `resolveImagePath.ts` uses `import.meta.glob('../assets/images/**/*', { eager: true })`, which bundles every file in that tree into the build regardless of whether any component actually renders it — there's no dead-code elimination for unused images. Run `npm run check:images` (`scripts/check-unused-images.mjs`) before adding new images or after removing a page/section that used some — it scans every `resolveImagePath()` call (both quote styles) and CSS `url(...)` reference and reports anything on disk that nothing points to, plus the reverse (a code reference with no file on disk). Non-zero exit if it finds either.
